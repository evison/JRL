...
public:
    ...
    corp(corp), K(K), latentReg(latentReg), lambda(lambda)
  {
    ...
    train_per_user = new std::map<int,long long>[nUsers];
    test_per_user = new std::map<int,long long>[nUsers];
    train_samples = new std::map<int, int>[nUsers];
    loss_samples = new std::map<int, int>[nUsers];
  ...
  double prediction_ui(int user, int beer);   
  double prediction_ui_bpr(int user, int beer);
  ...
  void precision(double* test, double* std, int flag);
  void normalizeWordWeights(void);
  void save(char* modelPath, char* predictionPath);
  void generate_train_samples();
  void generate_loss_samples();
  ...
  std::map<int,long long>* train_per_user;
  //map<int,long long>* pos_per_item;
  std::map<int,long long>* test_per_user;
  //map<int,long long>* test_per_item;   
  std::map<int, int>* train_samples;
  std::map<int, int>* loss_samples;
